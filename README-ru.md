...
Автоматический поиск в фильтрах по I18n полям
---------------------------------------------
Плагин содержит класс dinFormFilterDoctrine, который наследует стандартный sfFormFilterDoctrine и
предоставляет автоматический поиск по I18n полям в таблицах. Теперь не нужно создавать никаких
отдельных методов для таких полей. Для автоматического включения этого класса достаточно
первоначально сгенерировать классы фильтров командой
    php symfony doctrine-ext:build --filters
Если же у вас уже были ранее сгенерированы классы фильтров, то необходимо вручную прописать в классе
BaseFormFilterDoctrine вашего проекта наследование dinFormFilterDoctrine.

Уникальные индексы в I18n таблицах
----------------------------------
Плагин содержит измененный бихевиор I18nMod, которым можно заменить оригинальный во всех моделях
при помощи конфигурационного файла model.yml. Суть изменений состоит в добавлении поддержки
уникальных индексов для полей в I18n, с учетом языков разумеется.
Использовать очень просто - в конфиг бихевиора добавляется опция "unique", содержащая массив полей,
...
В данном случае будет создан составной уникальный ключ lang + title.

Создание гибких запросов
------------------------
Плагин содержит также средства создания запросов, учитывающих такие возможные изменения структуры
данных, как отключение или изменение I18n бихевиоров, изменение состава полей в таблицах и другие.
Для этого добавлена серия методов в класс dinDoctrineTable, который является базовым для классов
таблиц (при первой генерации моделей наследование классами таблиц осуществляется автоматически).

**Методы:**

**addSelect** принимает массив с названиями полей для добавления в запрос. Поля, привязанные к
I18n таблице будут распознаны и привязаны к соответствующему алиасу автоматически.

**addWhere** создана для добавления условий в запрос. Как и предыдущая, автоматически определяет,
относится ли поле к основной или к I18n таблице.

**addOrderBy** позволяет задавать в запросе сортировку. Также автоматически распознает I18n поля.

**getColumn** возвращает строку поля с нужным алиасом, в зависимости от того, к какой таблице
относится поле.

**joinI18n** присоединяет I18n таблицу к запросу.
